    ...
    def plot_braking_zones_by_track(
        ...
        for track_name, track_df in df.groupby("track", sort=False):
            ...
            if not zone_rows.empty:
                ...
                curv_col = "c" if "c" in zone_rows.columns else ("curvature" if "curvature" in zone_rows.columns else None)
                curv_vals = zone_rows[curv_col].to_numpy() if curv_col else np.full(len(zone_rows), np.nan)
